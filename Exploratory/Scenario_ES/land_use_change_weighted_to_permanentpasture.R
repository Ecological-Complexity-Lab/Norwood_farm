################### CHANGE IN LAND USE SIMULATION (Permanent pastures) ##################################
...
##### -- Semi - intensive (replace "WD","RG","MH","NH"and "GM" for "PP")
...
# create habitats PP to replace WD,RG, MH,NH and GM (WD,RG,MH and NH were created before)
...
# calculate changes in the area between PP and the habitat to replace 
converted_area<-rbind(WD_PP, RG_PP, MH_PP, NH_PP, GM_PP) %>% left_join(habitat_area, 
                                                                by = c("prev_hab" = "HabitatCode")) %>% 
  left_join(habitat_area, by = c("hab" = "HabitatCode")) %>% 
  select(node_from,node_to,weight,new_hab,prev_hab,hab,area_ave.x,area_ave.y) %>% 
  rename("area_prev_hab" ="area_ave.x", "area_PP" = "area_ave.y") %>% 
...
##### -- Intensive (replace "WD","RG","MH","NH","GM" and "SF" for "PP")
...
# calculate changes in the area between PP and the habitat to replace 
converted_area<-rbind(WD_PP, RG_PP, MH_PP, NH_PP, GM_PP, SF_PP) %>% left_join(habitat_area, 
                                                                       by = c("prev_hab" = "HabitatCode")) %>% 
  left_join(habitat_area, by = c("hab" = "HabitatCode")) %>% 
  select(node_from,node_to,weight,new_hab,prev_hab,hab,area_ave.x,area_ave.y) %>% 
  rename("area_prev_hab" ="area_ave.x", "area_PP" = "area_ave.y") %>% 
...
#write.csv(direct_ES,"Data/Land_use_dir_weighted_PP.csv", row.names= FALSE)
...
#write.csv(ratio_direct,"Data/Land_use_rat_dir_weighted_PP.csv", row.names= FALSE)
...
#write.csv(Indirect_1hop_landuse_weighted_2,"Data/Land_use_ind_1hop_weighted_PP.csv", row.names= FALSE)



#### - Calculate indirect effects considering 2 hops (node 1 - node 2 - node 3, effect of node 1 on node 3'E(D)S via node 2)

Indirect_1hop<-read.csv("Data/Land_use_ind_1hop_weighted_PP.csv",
...
#write.csv(Indirect_2hop,"Data/Land_use_ind_2hop_weighted_PP.csv", row.names= FALSE)

Indirect_2hop<-read.csv("Data/Land_use_ind_2hop_weighted_PP.csv",
                        sep =",") #load dataframe of indirect effects using 1 hop
...
#write.csv(I_ES,"Data/Land_use_ind_weighted_PP.csv")
...
hop_1 <- read.csv("Data/Land_use_ind_weighted_PP.csv", sep =",", row.names = 1) %>% 
...
hop_2 <- read.csv("Data/Land_use_ind_weighted_PP.csv", sep =",",row.names = 1) %>% 
...
#write.csv(output_ES,"Data/Land_use_output_weighted_PP.csv", row.names= FALSE)

##### -- Indirect provision Ratio output +/-
output_ind_ES <- read.csv("Data/Land_use_output_weighted_PP.csv", sep =",") 
...
#direct
direct_ES<- read.csv("Data/Land_use_dir_weighted_PP.csv", sep =",") 
...
ratio_direct<-read.csv("Data/Land_use_rat_dir_weighted_PP.csv")
