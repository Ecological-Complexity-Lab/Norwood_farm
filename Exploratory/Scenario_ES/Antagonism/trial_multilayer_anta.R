...
edges <- tibble(read.csv("Data/elist_nore_anta.csv")) 
...
nodes <- tibble(read.csv("Data/nodes_anta.csv")) #previous dataframe containing node's information
...
# remove from 02FV those species that belong to the group 15FVOTHERS because we assume they not provide pollination
services_anta<- services %>%  filter(lower%in%nodes$node_name)

# rearrange dataframe and add taxon
services_anta2<-services_anta %>%  rename (node_name = lower, ESS = upper) %>%
...
phy_node_atr_tax_es<- dplyr::left_join(nodes, services_anta2, by = "node_name") %>% select(-taxon.x,-node_id, -taxon.y) %>% 
...
#saveRDS(Norwood_farm, file="Norwood_farm_anta.RData")
