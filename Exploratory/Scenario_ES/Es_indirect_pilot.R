...
list_nodes_ES_provi<-nodes_ES %>% ungroup() %>% select(-layer_name) %>%
  filter (value ==1) %>% unique #nodes that provide ES
...
## add inverted links (to make the code easier to program. It will not affect the results)
...
rows_birds_butt<- Indirect_1hop %>%
  filter(taxon_from == "Butterfly" | taxon_from == "Seed-feeding bird") %>%
  distinct(layer, node_from, node_to, .keep_all = TRUE) # new subset after eliminating duplicate rows for node_from = birds and butterflies

int_without<-Indirect_1hop %>% filter(!(taxon_from == "Butterfly" | 
...
Indirect_1hop_final<-rbind(rows_birds_butt,int_without)#final dataframe containing indirect effects on ES via 1 hop

Indirect_1hop<-Indirect_1hop_final
...
rows_birds_butt<- Indirect_1hop_agg %>%
  filter(taxon_from == "Butterfly" | taxon_from == "Seed-feeding bird") %>%
  distinct(layer, node_from, node_to, .keep_all = TRUE) # new subset after eliminating duplicate rows for node_from = birds and butterflies

int_without<-Indirect_1hop_agg %>% filter(!(taxon_from == "Butterfly" | 
                                          taxon_from == "Seed-feeding bird")) #eliminate the interactions containing node_from =birds or butterflies from the original dataframe


Indirect_1hop_agg_final<-rbind(rows_birds_butt,int_without)#final dataframe containing indirect effects on ES via 1 hop

Indirect_1hop_agg<-Indirect_1hop_agg_final
