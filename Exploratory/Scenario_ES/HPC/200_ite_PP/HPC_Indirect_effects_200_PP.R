...
data <- read.csv("./ind_1hop_sim_PP.csv") %>% filter(management == mngmnt)
...
ind_row <- function(df, row) { #for the row
  ...
  df %>% 
    dplyr::filter(node_from == j, node_to != df$node_from[row],  #filter to avoid counting the interaction from node 2 to node 1 because the edgelist is directed 
           iteration == k) %>% # Filter dataframe (filter node 3's ES affected by node 2)
...
output_file <- paste("Results/", mngmnt, "_ind_2hop_sim_250_PP.csv", sep = "")
